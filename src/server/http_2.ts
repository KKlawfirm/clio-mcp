...
import { saveTokens, loadTokens, clearTokens as clearStoredTokens } from "../auth/tokenStorage.js";
...
function buildSessionContext(record: SessionRecord, sessionId: string): SessionContext {
  return {
    ...
    getAccessToken: async () => {
      // A new MCP session starts with no tokens in memory. Fall back to the
      // encrypted store on disk, so a restart or an unattended run does not
      // require a fresh browser authorisation.
      if (!record.tokens) {
        const persisted = await loadTokens().catch(() => null);
        if (persisted) record.tokens = persisted;
      }
      if (!record.tokens) {
        throw new Error(
      ...
      if (Date.now() > record.tokens.expires_at - 5 * 60 * 1000) {
        ...
        await saveTokens(record.tokens).catch(() => {});
      }
      return record.tokens.access_token;
    },
    storeTokens: (tokens: ClioTokens) => {
      record.tokens = tokens;
      void saveTokens(tokens).catch(() => {});
    },
    getTokens: () => record.tokens,
    clearTokens: () => {
      record.tokens = null;
      void clearStoredTokens().catch(() => {});
    },
...
export function createApp(auth: HttpAuthConfig): express.Express {
      ...
      if (!incomingSessionId) {
        ...
        const transport = new StreamableHTTPServerTransport({
          ...
          onsessioninitialized: async (sessionId) => {
            ...
            // Hydrate from the encrypted store on disk, so a session opened
            // after a restart — or by an unattended scheduled run — starts
            // already authenticated instead of demanding a browser login.
            if (!record.tokens) {
              record.tokens = await loadTokens().catch(() => null);
            }
  ...
  app.get("/oauth/callback", async (req, res) => {
    ...
    try {
      ...
      record.tokens = tokens;
      await saveTokens(tokens).catch((err: any) => {
        console.error("[http] could not persist tokens:", err?.message);
      });
